...
export class ItemUIComponent implements OnInit {
  ...
  // Method to handle Save button click in edit Item dialog
  onSaveClick(): void {
    console.log(this.data);
    this.http.put(ITEM_SERVICE, this.data.id, httpOptions).subscribe(
      (data) => {
        console.log(data);
        this.data = new Item();
        this.displayDialog = false;
  ...
  // Method to handle Delete button click in edit Item dialog
  onDeleteClick(): void {
    console.log(this.data);
    this.http.delete(ITEM_SERVICE + this.data.id, httpOptions).subscribe(
      (data) => {
        console.log(data);
        this.data = new Item();
        this.displayDialog = false;
        this.messageService.add({ severity: 'success', summary: 'Success Message', detail: 'Item deleted successfully.' });
        this.retrieveItems();
      },
      (error) => {
        console.log(error);
        this.messageService.add({ severity: 'error', summary: 'Error Message', detail: 'Item not deleted.' });
      }
    );
  }
