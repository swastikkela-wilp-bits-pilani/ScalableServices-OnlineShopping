...
export class SalesOrderUIComponent implements OnInit {
  ...
  // Method to handle Delete button click in update Order dialog
  onDeleteClick(): void {
    console.log(this.data);
    this.http.delete(SALESORDER_SERVICE + this.data.id, httpOptions).subscribe(
      (data) => {
        console.log(data);
        this.data = new SalesOrder();
        this.displayDialog = false;
        this.messageService.add({ severity: 'success', summary: 'Success Message', detail: 'rder deleted successfully.' });
        this.retrieveSalesOrders();
      },
      (error) => {
        console.log(error);
        this.messageService.add({ severity: 'error', summary: 'Error Message', detail: 'Order not deleted.' });
      }
    );
  }
